...
class JobStoreSimpletonMigration extends CustomTaskChange {

  val Description = "WORKFLOW_EXECUTION + EXECUTION + SYMBOL + JOB_STORE -> JOB_STORE_RESULT_SIMPLETON"
  // GOTC (substituting COUNT(*) for the projection): 1 row in set (5.22 sec)
  val QueryOutputsForDoneCallsInRunningWorkflows =
  """
      SELECT
        js.JOB_STORE_ID,   -- 1
        s.NAME,            -- 2
