...
class EngineJobExecutionActor(jobKey: BackendJobDescriptorKey,
  ...
  when(RunningJob) {
    ...
    case Event(hashes: CallCacheHashes, EmptyPartialCompletionData) =>
      log.info(s"Got hashes for ${jobKey.mkTag(workflowId)}, awaiting job completion")
    ...
    case Event(response: SucceededResponse, EmptyPartialCompletionData) if effectiveCallCachingMode.writeToCache =>
      log.info(s"Got job result for {}, awaiting hashes", jobKey.mkTag(workflowId))
  ...
  private def recordHashError(reason: Throwable) = {
    log.error(reason, "{}: hash error: {}. Disabling call caching for this job", tag, reason.getMessage)
