...
object EngineJobHashingActorSpec extends MockitoSugar {
  ...
  (
    ...
    dockerHashLookupActor: Option[ActorRef] = None,
    ...
    system.actorOf(EngineJobHashingActor.props(replyTo, jobDescriptor, initializationData, fileHashingActor.getOrElse(emptyActor), callCacheReadActor, dockerHashLookupActor.getOrElse(emptyActor), runtimeAttributeDefinitions, backendName, activity))
  }

  def emptyActor(implicit actorSystem: ActorSystem) = actorSystem.actorOf(Props.empty)

  def templateJobDescriptor
  (
    inputs: Map[LocallyQualifiedName, WdlValue] = Map.empty
  ) = {
